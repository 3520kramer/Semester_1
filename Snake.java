...
public class Snake {
   ...
   //Opretter en kontruktør for Snake
   public Snake(){
   ...
   public void move(){
      ...
      int oldY = this.y;           
      boolean outOfBounds = true;   //Opretter en variabel for at sørge fo r at spillet 
      while(outOfBounds){           //Opretter et while loop som kører så længe outOfBounds er true
         ...
         if(this.x <= 10 && this.y <= 10 && this.x >= 1 && this.y >= 1){
            System.out.println("Jeg er den onde slange og jeg står på " + this.x + "," + this.y);
            outOfBounds = false;
         }else{
            this.x = oldX;
            this.y = oldY;
   ...
   public boolean iEatYou(Rabbit rabbit){ // // // // // // // 
      if(this.x == rabbit.x && this.y == rabbit.y){
         System.out.println("Jeg spiser dig!");
         return true;
      }
      return false;
   }
